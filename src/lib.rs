...
impl RandomName {
    ...
    pub fn new() -> RandomName {
        let mut rng = match OsRng::new() {
            Ok(rng) => rng,
            Err(_) => return RandomName { name: "EDWIN ERROR".to_owned() },
        };
...
pub fn create_name<T: Rng>(rng: &mut T) -> String {
    let name: &str = rng.choose(NAMES).map(|x| *x).unwrap_or(NO_NAMES_NAME);
    let surname: &str = rng.choose(SURNAMES).map(|x| *x).unwrap_or(NO_NAMES_SURNAME);
    format!("{} {}", name, surname)
}
